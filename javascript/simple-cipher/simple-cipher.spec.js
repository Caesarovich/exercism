...
describe("Random key cipher", () => {
	...
	test("can encode", () => {
		// Here we take advantage of the fact that plaintest of "aaa..."
	...
	test("is reversible", () => {
		...
		// the same plaintest encode parameter as a result of the decode method
		const plaintest = "abcdefghij";
		expect(cipher.decode(cipher.encode(plaintest))).toEqual(plaintest);
	});
